...
<section class="single_page">						
    ...
    <div class="row">
        ...
        <div class="col-md-9 col-sm-8">				
            @foreach ($subcateposts as $subcatepost)
				
            <div class="archive_post_sec_again">
                <div class="row">
                    <div class="col-md-4 col-sm-5">
                        <div class="archive_img_again">
                            <a href="{{route('post.single',$subcatepost->id)}}"><img src="{{asset($subcatepost->image)}}" alt="Notebook"></a>
                        </div>
                    </div>
                    <div class="col-md-8 col-sm-7">
                        <div class="archive_padding_again">
                            <div class="archive_heading_01">
                                <a href="{{route('post.single',$subcatepost->id)}}">
                                    @if (session()->get('lang')  == 'english')
                                    {{$subcatepost->title_en}}
                                    @else
                                    {{$subcatepost->title_ar}}
                                    @endif
                                    </a>
                            </div>
                            <div class="archive_dtails">
                                @if (session()->get('lang')  == 'english')
                                {{Str::limit($subcatepost->details_en, 200)}}
                                @else
                                {{Str::limit($subcatepost->details_ar, 200)}}
                                @endif
                            </div>
                            <div class="dtails_btn"><a href="{{route('post.single',$subcatepost->id)}}">
                                @if (session()->get('lang')  == 'english')
                                Read More...
                                @else
                                أقرا أكثر...
            ...
            @endforeach
            {{ $subcateposts->links('pagination-links') }}
        </div>		
